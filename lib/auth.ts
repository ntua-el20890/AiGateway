      ...
      async authorize(
        ...
        if (isValid) {
          ...
          const token = jwt.sign(
            ...
            process.env.AUTH_SECRET || "your-jwt-secret-key", // Secret
            { expiresIn: "1d" } // Token expiration
          );

          // console.log("Generated token in authorize:", token); // Debug log
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      ...
      if (token && session.user) {
        ...
        session.accessToken = token.accessToken as string; // Add the token to the session`
      }
      return session;
    },
  },
